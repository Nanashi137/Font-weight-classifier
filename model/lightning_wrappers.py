...
class DeepFontWrapper(L.LightningModule):
    def __init__(self, model: DeepFont, num_classes: int, learning_rate: float = 0.01):
        ...
        self.model = model
        self.f1 = F1Score(task="multiclass", num_classes=num_classes, top_k=1)
        self.accuracy = Accuracy(task="multiclass", num_classes=num_classes, top_k=1)
        self.precision = Precision(task="multiclass", num_classes=num_classes, top_k=1)
        self.recall = Recall(task="multiclass", num_classes=num_classes, top_k=1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("train_pre", train_f1, prog_bar=True)
        self.log("train_rec", train_f1, prog_bar=True)
        return loss
